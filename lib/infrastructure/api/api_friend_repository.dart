...
class ApiFriendRepository implements FriendRepository {
  ...
  Future<List<DummyUser>> getFriends() async {
    final data = await _getJson(Uri.parse('$_apiBaseUrl/friendships'), auth: true);
    final friendsData = data['friends'] as List<dynamic>? ?? [];
    return friendsData.map((json) => _parseUser(json as Map<String, dynamic>)).toList();
  }
  ...
  Future<List<DummyUser>> searchUsers(String keyword) async {
    final query = Uri.encodeQueryComponent(keyword.trim().replaceFirst('@', ''));
    ...
    return usersData.map((json) => _parseUser(json as Map<String, dynamic>)).toList();
  }
  ...
  DummyUser _parseUser(Map<String, dynamic> userMap) {
    return DummyUser(
      ...
      name: userMap['name'] as String? ?? userMap['username'] as String? ?? '名無しさん',
